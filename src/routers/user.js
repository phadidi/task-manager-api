...
const upload = multer({
    ...
    fileFilter(req, file, callback) {
        if (!file.originalname.match(/\.(jpg|jpeg|png|jfif)$/)) {
            return callback(new Error('Please upload a non-GIF image.'))
        }
...
router.patch('/users/me', auth, async (req, res) => {
    ...
    if (!isValidOperation)
        return res.status(400).send({error: 'Contains disallowed parameters, cannot update'})
...
router.get('/users/:id/avatar', async (req, res) => {
    try {
        ...
        if (!user || !user.avatar)
            throw new Error('User or avatar doesn\'t exist')
